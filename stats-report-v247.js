(()=>{
'use strict';
if(window.MAGI_STATS_REPORT_V247)return;
const VERSION='v247';
...
function aggregateBreakdown(all,target,y,type){
 const np=n(target),dim=type==='order'?['打順']:['相手校','対戦相手','対戦校'];
 const detail=all.filter(r=>/\.xlsm$/i.test(String(r.fileName||''))&&season(r)===y&&/^打撃詳細$/i.test(String(r.sheetName||'').trim())&&n(player(r))===np);
 const groups=new Map();
 for(const r of detail){
  const label=raw(r,dim);if(!label)continue;
  ...
  const gameKey=[raw(r,['開催日','日付']),raw(r,['大会名']),raw(r,['試合順','試合']),raw(r,['相手校','対戦相手'])].join('|');
  g.games.add(gameKey);
  for(const m of [
...
function collect(q){
 ...
 const records=ys.map(y=>{
  ...
  calculatedRates(merged.values);
  return{season:y,...merged,orderRows:aggregateBreakdown(all,target,y,'order'),opponentRows:aggregateBreakdown(all,target,y,'opponent'),row:candidates[0]||null};
 }).filter(x=>x.row);
...
function installStyles(){
 if(document.getElementById('magi-stats-report-v247-style'))return;
...
window.MAGI_STATS_REPORT_V247=true;
