(()=>{
'use strict';
if(window.MAGI_STATS_REPORT_V248)return;
const VERSION='v248';
...
function collect(q){
 ...
 const records=ys.map(y=>{
  ...
  const risp=candidates.filter(isRispSheet).filter(r=>raw(r,['得点圏','得点圏打率'])!=='').sort((a,b)=>completeness(b)-completeness(a))[0]||null;
  const values=total?{...total.values}:{};
  values.rispAvg=risp?raw(risp,['得点圏','得点圏打率']):'';
  const sources={};
  for(const m of METRICS){
   if(m.key==='rispAvg'&&risp&&values[m.key]!=='')sources[m.key]={file:risp.fileName||'',sheet:risp.sheetName||''};
   else if(total&&values[m.key]!==''&&values[m.key]!==undefined)sources[m.key]={file:total.file||'',sheet:'打撃詳細から通算再集計'};
  }
  const sourceFiles=[...new Set(Object.values(sources).map(x=>x.file).filter(Boolean))];
  const sourceSheets=[...new Set(Object.values(sources).map(x=>x.sheet).filter(Boolean))];
...
function installStyles(){
 if(document.getElementById('magi-stats-report-v248-style'))return;
...
window.MAGI_STATS_REPORT_V248=true;
