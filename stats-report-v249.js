(()=>{
'use strict';
if(window.MAGI_STATS_REPORT_V249)return;
const VERSION='v249';
...
function aggregateBreakdown(all,target,y,type){
 ...
 const detail=all.filter(r=>/\.xlsm$/i.test(String(r.fileName||''))&&(!y||season(r)===y)&&/^打撃詳細$/i.test(String(r.sheetName||'').trim())&&n(player(r))===np);
...
function collect(q){
 ...
 return{target,records:[career,...seasonRecords.map(r=>({...r,suppressBreakdown:true}))],mode:'career',wanted:''};
}
function fmt(v,kind){
...
function reportMarkup(result){
 ...
 const sections=result.records.map(rec=>{
  ...
  const headType=rec.isCareer?'CAREER':'SEASON',headLabel=rec.isCareer?'全年度通算':rec.season,badge=rec.isCareer?'全年度再集計':'年度別';
  const sectionLabel=rec.isCareer?'選手通算成績':'年度成績';
...
window.MAGI_STATS_REPORT_V249=true;
