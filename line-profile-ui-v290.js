(()=>{
  ...
  function styles(){
    ...
    s.textContent=`
      ...
      .magiProfileForm input,.magiProfileForm select{width:100%;min-height:50px;box-sizing:border-box;border:1px solid #315574;border-radius:11px;background:#071827;color:#fff;padding:0 13px;font-size:16px}
      .magiProfileForm select{appearance:auto}
  ...
  function renderProfile(state){
    ...
    root.innerHTML=`<div class="magiAuthCard">
      ...
      <h1>初回利用の申請</h1>
      <p class="magiAuthLead">LINEの表示名だけでは本人を確認できない場合があるため、最初に氏名と立場を登録してください。</p>
      ...
      <form class="magiProfileForm">
        ...
        <label for="magiRequestedRole">立場</label>
        <select id="magiRequestedRole" name="requestedRole" required>
          <option value="">選択してください</option>
          <option value="coach">顧問・指導者</option>
          <option value="player">選手</option>
          <option value="member">保護者・その他</option>
        </select>
        <button class="magiProfileSubmit" type="submit">申請する</button>
        <div class="magiProfileError"></div>
      </form>
      <div class="magiProfileHint">LINEは同じ人かどうかを識別するために使います。管理者は「申請氏名・LINE表示名・LINEアイコン」を確認して利用を承認します。</div>
    ...
    form.addEventListener('submit',async(e)=>{
      ...
      if(!requestedRole){errorEl.textContent='立場を選択してください。';return;}
      button.disabled=true;
      errorEl.textContent='';
      try{
        const r=await fetch('/api/auth/line/profile',{
          method:'POST',headers:{'Content-Type':'application/json'},credentials:'same-origin',
          body:JSON.stringify({formalName,requestedRole})
        });
        const d=await r.json().catch(()=>({}));
  ...
  async function enhanceMemberPanel(panel){
    ...
    try{
      ...
      list.innerHTML=members.map(member=>{
        ...
        const options=['admin','coach','player','member'].map(r=>`<option value="${r}"${r===role?' selected':''}>${roleLabel(r)}</option>`).join('');
        ...
        return `<div class="magiMemberRow" data-id="${esc(member.id)}">
          <div class="magiMemberProfileMain">${member.picture_url?`<img class="magiMemberAvatar" src="${esc(member.picture_url)}" alt="">`:''}<div class="magiMemberProfileText">
            ...
            <div class="magiMemberRequest">${member.status==='pending'?'申請した立場':'現在の立場'}：${esc(roleLabel(role))} ・ ${esc(statusLabel(member.status))}</div>
          </div></div>
      ...
      list.querySelectorAll('[data-action]').forEach(button=>{
        button.addEventListener('click',async()=>{
          ...
          button.disabled=true;
          try{await memberRequest({id,role,status}); await enhanceMemberPanelRefresh(panel);}
