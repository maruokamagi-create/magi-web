(()=>{
'use strict';
if(window.MAGI_APP_BOOTSTRAP_V360)return;
window.MAGI_APP_BOOTSTRAP_V360=true;
const REV='360';
...
function setError(message){failed=true;ready=false;quarantineButton();const b=findButton();if(b){b.disabled=true;b.textContent='MAGI準備エラー'}const st=document.getElementById('status');if(st)st.textContent=`正式MAGIを起動できませんでした：${message}`;console.error('[MAGI bootstrap v360]',message);}
...
function semanticReady(){return typeof window.MAGI_SEMANTIC_RUN_V1==='function'&&String(window.MAGI_CLIENT_VERSION||'')==='360';}
...
async function boot(){
  ...
  window.MAGI_APP_RUNTIME=Object.freeze({version:'canonical-app-v360',clientVersion:360,ready:true,entry:'GEMINI_SEMANTIC_FIRST',formalRunner:'V4_UI_TRUTH_FAIL_CLOSED',progress:'EVENT_DRIVEN_ONLY',numericFailClosed:true,uiTruthFailClosed:true,staleClientFailClosed:true,localFallback:false,engineVersion:String(window.MAGI_ENGINE_V1?.version||'')});
