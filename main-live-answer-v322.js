(()=>{
'use strict';
if(window.MAGI_MAIN_LIVE_ANSWER_V322)return;
...
function ensurePanel(){
 ...
 p.innerHTML='<div style="font-size:11px;letter-spacing:.14em;color:#8db0d1;margin-bottom:8px">MAGI VERIFIED LIVE ANSWER</div><div id="magiLiveAnswerTextV322" style="font-size:17px;font-weight:850;line-height:1.65"></div><div id="magiLiveAnswerMetaV322" style="margin-top:10px;font-size:11px;line-height:1.65;color:#a9bdd0"></div>';
 card.insertAdjacentElement('afterend',p);
 return p;
}
function hidePanel(){const p=$('magiLiveAnswerV322');if(p)p.style.display='none'}
function showPanel(result,totalMs){
 const p=ensurePanel();if(!p)return false;
 const answer=$('magiLiveAnswerTextV322'),meta=$('magiLiveAnswerMetaV322');
 ...
 if(result?.cacheHit)bits.push('解析済みキャッシュ');
...
async function requestLive(question){
...
async function postLive(question){
 ...
 for(let attempt=0;attempt<3;attempt++){
  try{return await requestLive(question)}catch(e){last=e;if(e?.status===401||e?.status===403)throw e;if(attempt<2)await new Promise(r=>setTimeout(r,700*(attempt+1)))}
 }
...
function callLegacy(){const fn=window.runMagi;if(typeof fn==='function'&&!fn.__magiMainLiveCaptureV322)return fn.call(window);return undefined}
async function execute(btn){
 ...
 try{
  ...
  console.warn('[MAGI main live answer v322]',e?.message||e);
