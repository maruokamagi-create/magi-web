(()=>{
'use strict';
if(window.MAGI_MAIN_LIVE_ANSWER_V326)return;
window.MAGI_MAIN_LIVE_ANSWER_V326=true;
...
const DECISION_RE=/審議|べき|どう思う|評価して|比較して|候補|打順|起用|固定|ベストオーダー|スタメン|クリーンナップ|中軸|主軸|適性|継投|先発させ|先発に|クローザー|抑え|ローテ|戦術|采配|任せる|推薦|提案/i;
const PITCH_RE=/投手|ピッチャー|投球|登板|防御率|奪三振|与四死球|与四球|WHIP|被安打|自責点|セーブ|投球回|投球数|勝敗|勝利|敗北/i;
const BAT_RE=/打撃|打率|出塁率|長打率|OPS|安打|本塁打|打点|盗塁|三振|犠打|犠飛/i;
const FULL_RE=/打撃成績|投手成績|成績一覧|一覧|通算|今期|今季|今シーズン|今年|年度|シーズン|直近|最近|レポート/i;
function fullStatsKind(q){
 q=txt(q);if(!q||DECISION_RE.test(q))return'';
 if(PITCH_RE.test(q)&&FULL_RE.test(q))return'pitch';
 if(!PITCH_RE.test(q)&&BAT_RE.test(q)&&FULL_RE.test(q))return'bat';
 return'';
}
function normalizedStatsQuery(q){
 let out=txt(q);
 if(/今期|今季|今シーズン|今年/.test(out)&&!/現チーム|新チーム|2026\s*[-–—_. /]?\s*2027/.test(out))out+=' 現チーム';
 if(!/出して|教えて|見せて|知りたい|表示|一覧|何|直近|最近|は[？?]?$/i.test(out))out+=' 一覧';
 return out;
}

function ensurePanel(){
 let p=$('magiLiveAnswerV326');if(p)return p;
 const judge=$('judge'),card=judge?.querySelector?.('.card');if(!judge||!card)return null;
 p=document.createElement('div');p.id='magiLiveAnswerV326';
 ...
 p.innerHTML='<div style="font-size:11px;letter-spacing:.14em;color:#8db0d1;margin-bottom:8px">MAGI VERIFIED ANSWER</div><div id="magiLiveAnswerTextV326" style="font-size:17px;font-weight:850;line-height:1.65"></div><div id="magiLiveAnswerMetaV326" style="margin-top:10px;font-size:11px;line-height:1.65;color:#a9bdd0"></div>';
 card.insertAdjacentElement('afterend',p);return p;
}
function hidePanel(){const p=$('magiLiveAnswerV326');if(p)p.style.display='none'}
function showPanel(result,totalMs){
 const p=ensurePanel();if(!p)return false;
 const answer=$('magiLiveAnswerTextV326'),meta=$('magiLiveAnswerMetaV326');
...
async function runFullStats(q,kind,btn){
 const input=$('q'),oldValue=input?.value,oldText=btn?.textContent,status=$('status');
 try{
  hidePanel();
  if(input)input.value=normalizedStatsQuery(q);
  if(btn){btn.disabled=true;btn.textContent=kind==='pitch'?'投手成績を集計中…':'打撃成績を集計中…';}
  if(status)status.textContent=kind==='pitch'?'投手成績照会：通算・年度別・相手校別を集計しています…':'打撃成績照会：通算・打順別・相手校別を集計しています…';
  const fn=formalRun();if(!fn)throw new Error('既存の成績レポート処理を呼び出せません');
  return await fn.call(window);
 }finally{
  if(input&&oldValue!==undefined)input.value=oldValue;
...
async function deliberateWithResolvedEvidence(result){
 ...
 if(packet?.text)window.searchDataEvidence=()=>packet;
 window.routeQuestion=()=>({type:packet?.text?'data':'judge',label:'質問理解 → 証拠取得 → 3賢人審議',help:packet?.text?'MAGIが質問から必要資料を特定し、その内容を審議材料として渡します。':'質問内容を3賢人の正式審議へ渡します。'});
...
async function execute(btn){
 ...
 if(busy)return;busy=true;const started=performance.now(),oldText=btn?.textContent;
 try{
  const statsKind=fullStatsKind(q);
  if(statsKind)return await runFullStats(q,statsKind,btn);
  if(btn){btn.disabled=true;btn.textContent='確認中…'}hidePanel();
  ...
  console.warn('[MAGI main core v326]',e?.message||e);
  ...
  if(btn){btn.disabled=false;if(btn.textContent==='確認中…')btn.textContent=oldText||'MAGI実行'}busy=false;
 }
}

document.addEventListener('click',event=>{const btn=mainButton(event.target);if(!btn)return;event.preventDefault();event.stopPropagation();event.stopImmediatePropagation();execute(btn);},true);
