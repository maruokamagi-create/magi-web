(()=>{
'use strict';
if(window.MAGI_APP_BOOTSTRAP_V362)return;
window.MAGI_APP_BOOTSTRAP_V362=true;
const REV='362';
...
function setError(message){failed=true;ready=false;quarantineButton();const b=findButton();if(b){b.disabled=true;b.textContent='MAGI準備エラー'}const st=document.getElementById('status');if(st)st.textContent=`正式MAGIを起動できませんでした：${message}`;console.error('[MAGI bootstrap v362]',message);}
...
function semanticReady(){return typeof window.MAGI_SEMANTIC_RUN_V2==='function'&&String(window.MAGI_CLIENT_VERSION||'')==='362';}
async function waitCapability(check){while(!check())await sleep(40);}
function activateButton(){const b=findButton();if(!b)throw new Error('MAGI実行ボタンを取得できません');b.id='magiRunButton';b.type='button';b.dataset.magiEntry='semantic-v362';b.removeAttribute('onclick');b.disabled=false;b.textContent='MAGI実行';}
async function boot(){
  ...
  await load(`/magi-formal-runner-v358.js?v=${REV}`);await waitCapability(formalReady);
  ...
  window.MAGI_APP_RUNTIME=Object.freeze({version:'canonical-app-v362',clientVersion:362,runtimeVersion:362,ready:true,entry:'WINDOW_CAPTURE_SEMANTIC_V2',formalRunner:'V4_UI_TRUTH_FAIL_CLOSED',progress:'EVENT_DRIVEN_ONLY',numericFailClosed:true,uiTruthFailClosed:true,authoritativeEvidenceVisible:true,staleClientFailClosed:true,bootstrapLast:true,localFallback:false,engineVersion:String(window.MAGI_ENGINE_V1?.version||'')});
