(()=>{
'use strict';
if(window.MAGI_DRIVE_PROGRESS_V238)return;
...
style.id='magi-drive-progress-v238-style';
style.textContent=`
...
.driveProgressNote{margin-top:7px;color:#8fa9bf;font-size:11px;line-height:1.45}
...
function infer(text){
  ...
  if(/重要データを再取得|重要データを自動再取得/.test(t)){render(99,'重要データを確認しています');return}
  if(/Google Drive接続済み：/.test(t)){render(100,'Google Driveの読み込みが完了しました','complete',t.replace(/^Google Drive接続済み：/,''));return}
  if(/Google Drive接続済み。重要データ/.test(t)){render(100,'Google Driveの確認が完了しました','complete',t);return}
  if(/取込完了：/.test(t)){render(100,'ファイルの読み込みが完了しました','complete',t.replace(/^取込完了：/,''));return}
}
...
window.MAGI_DRIVE_PROGRESS_V238=true;
window.MAGI_DRIVE_PROGRESS_VERSION='1.0';
})();