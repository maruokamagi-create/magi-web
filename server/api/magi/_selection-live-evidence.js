...
export const SELECTION_LIVE_EVIDENCE_VERSION = 'selection-live-evidence-v9-current-fielding-policy';
...
export async function buildCurrentSelectionEvidence({question,routed={},auditProvider=runDriveLiveAudit}={}){
  ...
  const summary=kind==='FULL_LINEUP'
    ? `現チーム14名の2026-2027通算正本を主評価にし、${recentSix.status==='COMPLETE'?'打撃詳細CSVから直近6試合を再集計し、':''}2025-2026の同14名の過去実績と、キャプテン・精神的支柱など確認済みのチーム内役割、現在の守備起用方針も重ねた1〜9番打順審議用Evidenceです。標準打順は相手別データ不足だけでは保留しません。`
    : kind==='PITCHING_PLAN'
  ...
  return {
    ...
    dataRule:'候補は現チーム14名のみ。現チーム通算を主評価し、過去実績は経験・再現性の重要な比較材料、直近6試合は短期状態、確認済みのチーム内役割は定性材料として重ねる。大久保 陽翔はキャプテン・精神的柱としての役割を加味し、不調時は下位降格だけでなく1番起用による打席増・復調促進も検討する。守備は大久保 陽翔を投手・遊撃・三塁の現行候補として扱い、外野起用は想定しない。標準ベストオーダーは相手別情報不足だけでは保留しない。ここにない数値・役割・選手・性格・将来結果は作らない'
  };
}
