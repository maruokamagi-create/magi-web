...
export const SELECTION_LIVE_EVIDENCE_VERSION = 'selection-live-evidence-v10-rationale-depth';
...
export async function buildCurrentSelectionEvidence({question,routed={},auditProvider=runDriveLiveAudit}={}){
  ...
  if(kind==='FULL_LINEUP'){
    lines.push(
      '【チーム内役割・指導方針】大久保 陽翔は現チームのキャプテンであり、成績だけでなく精神的な柱としての役割も打順判断の重要な定性材料として加味する。ただしキャプテンだから自動的に特定打順へ固定するのではなく、過去実績・今季通算・直近6試合と合わせて判断する。近藤先生の起用思想として、主軸打者が調子を落としている場合は単純に下位へ下げるだけでなく、1番に置いて打席数を増やし復調を促す戦術も有力案として検討する。大久保 陽翔を4番から動かす場合は「評価低下による降格」なのか「1番起用による復調促進」なのかを明確に区別して説明する。',
      '【現在の守備起用方針】大久保 陽翔は現在、投手・遊撃・三塁を守備候補とする。外野起用は想定しない。過去や今季のデータに外野守備の実記録が残っていても、現在の標準ベストオーダーでは外野へ配置しない。',
      ...
      '【運用ルール】1番〜9番は現チーム14名から異なる9名で構成する。3賢人は独立して全打順を作り、クロス審議では具体的な打順番号と選手名を挙げて互いの並びを検証した後に二次案を出す。標準ベストオーダーは相手投手の左右別データがなくても決定し、相手情報は後から微調整する条件として扱う。特に1〜5番は「過去実績 → 今季通算 → 直近6試合 → チーム内役割」の4層を比較して理由を説明する。中軸候補を別の選手より下げる場合は、3層の具体的数値に加えて役割面も比較し、その変更理由を説明できない限り変更理由として認めない。守備は実記録だけでなく確認済みの現在の起用方針を優先する。直近6のEvidenceがない場合は「好調」「不調」「最近上向き」などを作らない。旧チームの引退選手を打順に入れない。ここにない数値・性格・将来結果は作らない。'
    );
  ...
  const summary=kind==='FULL_LINEUP'
    ? `現チーム14名の2026-2027通算正本を主評価にし、${recentSix.status==='COMPLETE'?'打撃詳細CSVから直近6試合を再集計し、':''}2025-2026の同14名の過去実績と、キャプテン・精神的支柱など確認済みのチーム内役割、現在の守備起用方針も重ねた1〜9番打順審議用Evidenceです。標準打順は相手別データ不足だけでは保留せず、理由は具体的な数値比較まで示します。`
    : kind==='PITCHING_PLAN'
  ...
  return {
    ...
    dataRule:'候補は現チーム14名のみ。現チーム通算を主評価し、過去実績は経験・再現性の重要な比較材料、直近6試合は短期状態、確認済みのチーム内役割は定性材料として重ねる。ベストオーダーでは抽象理由だけを禁止し、主要打順は具体的な数値比較で説明する。大久保 陽翔はキャプテン・精神的柱としての役割を加味し、不調時は下位降格だけでなく1番起用による打席増・復調促進も検討する。守備は大久保 陽翔を投手・遊撃・三塁の現行候補として扱い、外野起用は想定しない。標準ベストオーダーは相手別情報不足だけでは保留しない。ここにない数値・役割・選手・性格・将来結果は作らない'
  };
}
