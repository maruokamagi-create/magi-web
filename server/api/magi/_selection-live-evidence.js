...
export const SELECTION_LIVE_EVIDENCE_VERSION = 'selection-live-evidence-v11-lineup-operating-policy';
...
export async function buildCurrentSelectionEvidence({question,routed={},auditProvider=runDriveLiveAudit}={}){
  ...
  const summary=kind==='FULL_LINEUP'
    ? `現チーム14名の2026-2027通算正本を主評価にし、${recentSix.status==='COMPLETE'?'打撃詳細CSVから直近6試合を再集計し、':''}2025-2026の同14名の過去実績と、確認済みの役割・守備方針、右投手基本対応、上位5人の基準線、6〜9番の変動運用、練習試合の柔軟運用を重ねた1〜9番打順審議用Evidenceです。`
    : kind==='PITCHING_PLAN'
  ...
  return {
    ...
    dataRule:'候補は現チーム14名のみ。標準ベストオーダーは基本右投手対応で、1〜5番は大野竜暉→坂田暉馬→嶋田栄志→大久保陽翔→中嶋玲月を現在の基準線とする。ただし極端な不調・怪我・投手守備事情・明確な新Evidenceがあれば変更する。大野は出塁率を1番適性で重く評価し、坂田・嶋田の小母数の高率を過大評価しない。6〜8番は直近状態・守備・先発投手で変動し、橋向結都は非登板時6番遊撃を有力案、先発時は下位へ。武田晴琉翔の9番左翼は守備・走力込みの有力候補だが固定しない。練習試合は柔軟、特に第2試合は実験枠として通常序列の根拠にしない。現チーム通算を主評価し、過去実績・直近6・確認済み役割を重ね、ここにない数値・役割・選手・性格・将来結果は作らない。'
  };
}
