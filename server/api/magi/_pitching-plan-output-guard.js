...
function evidenceText(caseData){return JSON.stringify(caseData?.evidence||{});}
...
function validatePitchingPlanText(caseData,value){
  ...
  const issues=[];
  const supplied=evidenceText(caseData);
  ...
  const hasStabilityEvidence=/(?:安定|安定性|継続性|ばらつき|試合別|登板別|標準偏差|連続.{0,8}(?:登板|試合))/.test(supplied);
  ...
  // “Enough sample for this role” needs an explicit threshold/baseline in Evidence.
  const hasSampleThreshold=/(?:十分な母数|必要な母数|母数基準|サンプル基準|最低.{0,12}(?:登板|投球回)|(?:登板|投球回).{0,12}(?:基準|閾値|必要数))/.test(supplied);
  ...
  const hasRoleEvidence=/(?:セーブ|クローザー.{0,12}(?:実績|起用|登板)|抑え.{0,12}(?:実績|起用|登板)|終盤.{0,12}(?:実績|起用|登板)|高レバレッジ|勝負どころ|プレッシャー)/.test(supplied);
  if(!hasRoleEvidence){
