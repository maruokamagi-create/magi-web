...
const ROUTER_SYSTEM = `
あなたは《MAGI》質問理解ルーター。役割は「回答すること」ではなく、ユーザーの自然言語質問の意味を理解し、次に実行すべき処理を1つだけ決めること。

最重要原則:
- キーワード一致ではなく、文全体の意味・依頼目的・省略・言い間違い・曖昧さを解釈する。
- 自信がないのに既存機能へ押し込まない。安全に実行できないなら CLARIFY を選ぶ。
- CLARIFY は失敗ではない。情報不足・複数解釈・指示対象不明・文脈参照不明のときの正解である。
- 質問が意味不明でも、無理に推測しない。
- 事実照会と判断依頼を厳密に分ける。数値・記録を「教えて/見せて/出して」は原則照会。起用・評価・比較優劣・べき論・戦術判断は DELIBERATION。
- PLAYER_COMPARISON は「数値や事実の比較」に使う。どちらを起用すべきか、誰が向くか、4番は誰か等の判断は DELIBERATION。
- 「投手成績」は必ず PITCHING_LOOKUP。「打撃成績」は必ず BATTING_LOOKUP。
- 「成績」だけで打撃/投手のどちらか確定できない場合は、勝手に打撃へ寄せず CLARIFY にする。
...
- 3賢人審議は route=DELIBERATION のときだけ必要。照会や資料検索で needsDeliberation=true にしない。
...
「陽翔どう？」=> 何についてか不足するので CLARIFY
「昨日のあれどうだった？」=> 文脈で一意に解決できなければ CLARIFY
「チームの通算勝敗を教えて」=> TEAM_LOOKUP
「8月号のTEAM REPORTを見せて」=> DOCUMENT_SEARCH
「MAGIって何？」=> GENERAL_QUESTION

understoodRequest は、質問を勝手に膨らませず1文で言い換える。
clarificationQuestion は、1回の聞き返しで最も情報量が増える短い日本語質問にする。
...
function normalizeContext(value) {
  ...
  return value.slice(-8).map((item, index) => {
    if (typeof item === 'string') return { role: 'user', text: canonicalizeKnownNameText(cleanText(item, 3000)) };
    const role = ['user','assistant','system'].includes(String(item?.role || '').toLowerCase())
      ? String(item.role).toLowerCase()
      : 'user';
    return { role, text: canonicalizeKnownNameText(cleanText(item?.text ?? item?.content, 3000)) };
  }).filter(x => x.text).map((x, index) => ({ ...x, index }));
}
...
function normalizeResult(raw) {
  ...
  let confidence = CONFIDENCE_SET.has(String(raw?.confidence || '').toUpperCase()) ? String(raw.confidence).toUpperCase() : 'LOW';
  ...
  for (const value of Array.isArray(raw?.players) ? raw.players : []) {
    ...
    } else if (!unresolved.includes(text)) {
      unresolved.push(text);
    }
  }

  let needsClarification = raw?.needsClarification === true || route === 'CLARIFY';
  ...
  if (needsClarification && !clarificationQuestion) {
    clarificationQuestion = '質問の対象や知りたい内容を、もう少し具体的に教えてください。';
  }
  if (!needsClarification) clarificationQuestion = '';
...
export default async function handler(req, res) {
  ...
  try {
    ...
    const question = canonicalizeKnownNameText(cleanText(body?.question, 4000));
    if (question.length < 1) return sendJson(res, 400, { error: 'question is required' });

    const suppliedContext = normalizeContext(body?.context);
    const result = await callGemini({
      ...
      userPayload: {
        ...
        instruction: 'Classify only. Do not answer the baseball question itself. When unsafe to execute exactly one route, choose CLARIFY.'
      },
      responseSchema
    });
    ...
    return sendJson(res, 200, {
      ok: true,
      routerVersion: 'v1-semantic-gemini',
      ...normalizeResult(result)
    });
  } catch (error) {
    console.error('[MAGI QUESTION ROUTER]', error?.message || error);
    return sendJson(res, 500, {
      ok: false,
      error: '質問理解ルーターでエラーが発生しました。',
      route: 'CLARIFY',
      needsDeliberation: false,
      needsClarification: true,
      clarificationQuestion: '質問をもう一度入力してください。'
    });
  }
}
