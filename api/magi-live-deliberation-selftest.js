...
function compactBatting(players){
  return (players||[]).map(p=>({
    name:p.name,games:p?.games??p?.batting?.GAMES??p?.batting?.G??'',PA:p?.batting?.PA??'',AB:p?.batting?.AB??'',H:p?.batting?.H??'',RBI:p?.batting?.RBI??'',AVG:p?.batting?.AVG??'',OBP:p?.batting?.OBP??'',SLG:p?.batting?.SLG??'',OPS:p?.batting?.OPS??'',RISP:p?.batting?.RISP??p?.batting?.RISP_AVG??'',BB:p?.batting?.BB??'',HBP:p?.batting?.HBP??'',SB:p?.batting?.SB??''
  }));
}
...
async function serialPersonaSet(base,phase,buildBody){const out={};for(const p of PERSONAS)out[p]=await post(base,'/api/magi/persona',buildBody(p),`${phase}_${p.toUpperCase()}`);return out;}
...
async function runOnce(base,packet){
  ...
  if(allSame(second))second=await doSecond('3賢人の二次打順が完全一致したため、多数派への同調を排除して独立再検証する。同じ案を維持する場合も代替案を比較した理由を明示する。');
