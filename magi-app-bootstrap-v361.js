(()=>{
'use strict';
if(window.MAGI_APP_BOOTSTRAP_V361)return;
window.MAGI_APP_BOOTSTRAP_V361=true;
const REV='361';
...
function setError(message){failed=true;ready=false;quarantineButton();const b=findButton();if(b){b.disabled=true;b.textContent='MAGI準備エラー'}const st=document.getElementById('status');if(st)st.textContent=`正式MAGIを起動できませんでした：${message}`;console.error('[MAGI bootstrap v361]',message);}
...
function evidenceHookReady(){return window.MAGI_EVIDENCE_HOOK_V361_META?.explicitAtExecution===true;}
function truthReady(){return window.MAGI_UI_TRUTH_V359_API?.version==='ui-truth-v359';}
function coreGuardReady(){return String(window.MAGI_RUNTIME_CLIENT_VERSION||'')==='361';}
function semanticReady(){return typeof window.MAGI_SEMANTIC_RUN_V1==='function'&&String(window.MAGI_CLIENT_VERSION||'')==='360'&&coreGuardReady();}
...
async function boot(){
  ...
  await load(`/magi-evidence-hook-v361.js?v=${REV}`);await waitCapability(evidenceHookReady);
  ...
  if(typeof guarded!=='function'||guarded?.meta?.uiTruthFailClosed!==true)throw new Error('UI truth guardを起動できません');
  window.MAGI_FORMAL_UI_RUNNER_V2=guarded;
  await load(`/magi-core-client-guard-v361.js?v=${REV}`);await waitCapability(coreGuardReady);
  await load(`/main-live-answer-v329.js?v=${REV}`);await waitCapability(semanticReady);
  ready=true;activateButton();
  window.MAGI_APP_RUNTIME=Object.freeze({version:'canonical-app-v361',clientVersion:360,runtimeVersion:361,ready:true,entry:'GEMINI_SEMANTIC_FIRST',formalRunner:'V5_EXECUTION_BOUND_EVIDENCE',progress:'EVENT_DRIVEN_ONLY',numericFailClosed:true,uiTruthFailClosed:true,staleClientFailClosed:true,bootstrapLast:true,localFallback:false,engineVersion:String(window.MAGI_ENGINE_V1?.version||'')});
