(()=>{
'use strict';
if(window.MAGI_STATS_REPORT_V246)return;
const VERSION='v246';
...
const METRICS=[
 ...
 {key:'rispAvg',label:'得点圏打率',aliases:['得点圏打率','打率'],kind:'rate'}
];
...
function compositeRecord(candidates,y){
 ...
 const risp=candidates.filter(isRispSheet).filter(r=>raw(r,['得点圏打率','打率'])!=='').sort((a,b)=>completeness(b)-completeness(a))[0]||null;
...
const BREAKDOWN_METRICS=[
 {key:'games',label:'試合'},{key:'pa',label:'打席'},{key:'ab',label:'打数'},{key:'h',label:'安打'},{key:'rbi',label:'打点'},{key:'avg',label:'打率'},{key:'obp',label:'出塁率'},{key:'slg',label:'長打率'},{key:'ops',label:'OPS'}
];
function dimensionValue(r,aliases){
 const direct=raw(r,aliases);if(direct)return direct;
 const skip=new Set([...NAME_ALIASES,...METRICS.flatMap(m=>m.aliases)].map(n));
 const cs=r?.columns||[],vs=r?.values||[];
 for(let i=0;i<cs.length;i++){const v=String(vs[i]??'').trim();if(v&&!skip.has(n(cs[i])))return v}
 return'';
}
function breakdownRows(candidates,type){
 const order=type==='order',re=order?/打順別/i:/相手校別|対戦相手別|相手別/i;
 const aliases=order?['打順','打順名','打順区分','打順番号']:['対戦相手','相手校','対戦校','相手','学校名','チーム名'];
 const found=candidates.filter(r=>re.test(String(r?.sheetName||''))).map(r=>{
  const label=dimensionValue(r,aliases);
  const values={};for(const m of BREAKDOWN_METRICS){const def=METRICS.find(x=>x.key===m.key);values[m.key]=def?raw(r,def.aliases):''}
  return{label,values,file:r.fileName||'',sheet:r.sheetName||''};
 }).filter(x=>x.label&&Object.values(x.values).some(Boolean));
 const seen=new Set();
 return found.filter(x=>{const k=n(x.label)+'|'+BREAKDOWN_METRICS.map(m=>x.values[m.key]).join('|');if(seen.has(k))return false;seen.add(k);return true});
}
function collect(q){
 ...
 if(!mine.length)return{error:`${target}の${wanted||'通算'}打撃成績をDrive正本のxlsmから取得できませんでした。`};
 const ys=[...new Set(mine.map(season))].sort();
 const records=ys.map(y=>{
  const candidates=mine.filter(r=>season(r)===y),merged=compositeRecord(candidates,y);
  return{season:y,...merged,orderRows:breakdownRows(candidates,'order'),opponentRows:breakdownRows(candidates,'opponent'),row:candidates[0]||null};
 }).filter(x=>x.row);
...
function reportMarkup(result){
 ...
 const sections=result.records.map(rec=>{
  ...
  const source=files+(sheets?` / 通算：${sheets}`:'');
  const order=breakdownMarkup('打順別成績','打順',rec.orderRows);
  const opponent=breakdownMarkup('相手校別成績','相手校',rec.opponentRows);
  return`<section class="statsSeason"><div class="statsSeasonHead"><div><span>SEASON</span><b>${esc(rec.season)}</b></div><div class="statsVerified">DRIVE正本</div></div><div class="statsSectionLabel">通算成績</div><div class="statsFeatured">${featured}</div><div class="statsMetrics">${cells}</div><div class="statsSource"><b>参照元</b> ${esc(source)}</div>${order}${opponent}</section>`;
 }).join('');
 return`<article id="statsPdfSource" class="statsPaper"><header class="statsHeader"><div class="statsBrand"><img src="/magi-official-symbol-v125.svg?v=140" alt=""><div><b>MAGI DATA REPORT</b><span>Maruoka Advanced Game Intelligence</span></div></div><div class="statsIssue">発行 ${esc(date)}</div></header><div class="statsTitle"><span>PLAYER BATTING RECORD</span><h2>${esc(result.target)}</h2><p>通算・打順別・相手校別 打撃成績</p></div>${sections}<footer class="statsFooter"><b>《MAGI》</b><span>丸岡中学校軟式野球部 データ資料</span><small>通算・打順別・相手校別はそれぞれ専用集計から取得し、相互に数値を混ぜていません。得点圏打率は「得点圏打率一覧」から取得しています。</small></footer></article>`;
}
function ensureUi(){
...
window.MAGI_STATS_REPORT_V246=true;
