(()=>{
'use strict';
if(window.MAGI_DRIVE_MEDIA_INDEX_V254)return;
window.MAGI_DRIVE_MEDIA_INDEX_V254=true;
...
async function autoPdf(){
 const targets=driveIndex.filter(f=>isPdf(f)&&!importedDriveFiles.has(f.id)&&(Number(f.size)||0)<=MAX_PDF_BYTES).slice(0,AUTO_PDF_LIMIT);
 for(let i=0;i<targets.length;i++){
  const file=targets[i];state.set(file.id,{kind:'working',label:`PDF自動読取 ${i+1}/${targets.length}`});renderFiles();
  try{await importMedia(file,{silent:true})}catch(e){state.set(file.id,{kind:'error',label:'PDF読取：再試行'})}
 }
 renderFiles();
}
...
window.scanDrive=async function(...args){
 ...
 updateLegend();await restoreCached();renderFiles();await autoPdf();
...
updateLegend();
window.MAGI_DRIVE_MEDIA_INDEX='v254';
})();
