...
function installEvidence(){
 ...
 const wrapped=function(q){const e=prev(q);if(!e||!relevant(q))return e;const rows=((typeof dataRecords!=='undefined'?dataRecords:window.dataRecords)||[]).filter(r=>r&&r.source==='drive');const integ=build(rows);window.MAGI_NUMERIC_INTEGRITY=integ;window.MAGI_NUMERIC_FACTS=integ.ok?integ.facts:{};const clean=stripOldSnapshot(e.text),block=integ.ok?trustedBlock(integ):conflictBlock(integ);e.text=block+'\n\n'+clean;e.numericIntegrity=integ;e.candidatePriority=block;e.summary=String(e.summary||'')+(integ.ok?' 数値整合性検証済み。':' 数値不一致のため審議停止。');return e;};
...
function installRunGate(){
 ...
 const wrapped=async function(...args){const q=(document.getElementById('q')?.value||'').trim();if(relevant(q)){let e=null;try{e=window.searchDataEvidence(q)}catch(_){e=null}const integ=window.MAGI_NUMERIC_INTEGRITY;if(!e||!integ||!integ.ok){stopCard(q,integ||{conflicts:['数値検証処理を完了できない']});return;}}return original.apply(this,args)};
