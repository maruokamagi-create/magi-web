(()=>{
'use strict';
if(window.MAGI_MAIN_LIVE_ANSWER_V320)return;
window.MAGI_MAIN_LIVE_ANSWER_V320=true;
// Keep the legacy click guard disabled too.
...
function ensurePanel(){
 ...
 p.innerHTML='<div style="font-size:11px;letter-spacing:.14em;color:#8db0d1;margin-bottom:8px">MAGI VERIFIED LIVE ANSWER</div><div id="magiLiveAnswerTextV320" style="font-size:17px;font-weight:850;line-height:1.65"></div><div id="magiLiveAnswerMetaV320" style="margin-top:10px;font-size:11px;line-height:1.65;color:#a9bdd0"></div>';
 card.insertAdjacentElement('afterend',p);
 return p;
}
function hidePanel(){const p=$('magiLiveAnswerV320');if(p)p.style.display='none'}
function showPanel(result){
 const p=ensurePanel();if(!p)return false;
 const answer=$('magiLiveAnswerTextV320'),meta=$('magiLiveAnswerMetaV320');
...
async function postLive(question){
 ...
 for(let attempt=0;attempt<3;attempt++){
  ...
  try{
   const r=await fetch('/api/magi/health',{method:'POST',credentials:'same-origin',cache:'no-store',headers:{'Content-Type':'application/json'},body:JSON.stringify({mode:'LIVE_QUESTION_ANSWER',question,context:[]}),signal:c.signal});
   let d=null;try{d=await r.json()}catch(_){d=null}
   if(r.ok)return d||{};
   const e=new Error(txt(d?.error)||`LIVE_QUESTION_ANSWER ${r.status}`);e.status=r.status;throw e;
  }catch(e){last=e;if(e?.status===401||e?.status===403)throw e;if(attempt<2)await new Promise(r=>setTimeout(r,700*(attempt+1)))}finally{clearTimeout(t)}
 }
...
function callLegacy(){
 ...
 if(typeof fn==='function'&&!fn.__magiMainLiveCaptureV320)return fn.call(window);
 return undefined;
}
async function execute(btn){
 ...
 try{
  ...
  if(shouldDisplay(r)){showPanel(r);return}
  if($('status'))$('status').textContent='この質問は従来のMAGI審議へ引き継ぎます…';
  return callLegacy();
 }catch(e){
  console.warn('[MAGI main live answer v320]',e?.message||e);
...
// Capture the actual production button. This survives loader body replacement and does not depend on runMagi being wrapped at the right time.
...
// Re-create the answer panel after the legacy loader replaces body contents.
