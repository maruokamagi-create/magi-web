(()=>{
  ...
  .answer .voteIcon{width:30px;height:30px;border-radius:50%;display:inline-block;flex:0 0 auto;box-shadow:0 2px 5px rgba(0,0,0,.20)}
  ...
  @media(max-width:430px){
    ...
    .answer .voteIcon{width:29px;height:29px}
  ...
  @media(max-width:380px){
    ...
    .answer .voteIcon{width:27px;height:27px}
    .answer .voteText{font-size:10px}
  }
  `;
  for(const id of ['magi-layout-v104-style','magi-layout-v105-style','magi-layout-v106-style','magi-layout-v107-style','magi-layout-v108-style']){const old=document.getElementById(id);if(old)old.remove()}
  const st=document.createElement('style');st.id='magi-layout-v109-style';st.textContent=css;document.head.appendChild(st);
  ...
  function decorateBadge(el){
    ...
    if(/◎|賛成/.test(raw)&&!/条件/.test(raw)){cls='yes';label='◎ 可決'}
    else if(/○|条件付き/.test(raw)){cls='cond';label='○ 条件付き可決'}
    else if(/×|✕|反対/.test(raw)){cls='no';label='× 否決'}
