(()=>{
'use strict';
if(window.MAGI_LINEUP_STATS_V368)return;
window.MAGI_LINEUP_STATS_V368=true;

let cache=null,pending=null;
const norm=s=>String(s??'').normalize('NFKC').replace(/[\s　・･_\-\/()（）\[\]【】]/g,'').toLowerCase();

function injectStyle(){
  if(document.getElementById('magi-lineup-stats-v368-style'))return;
  const s=document.createElement('style');
  s.id='magi-lineup-stats-v368-style';
  s.textContent=`
  ...
  @media(max-width:430px){
    .magiFinalDecisionRequestedStats{font-size:10.5px!important}
    .magiLineupStatsLine{gap:6px!important}
  }
  @media(max-width:360px){
    .magiFinalDecisionRequestedStats{font-size:10px!important}
...
function rowText(st){
  return `<div class="magiLineupStatsLine"><span class="magiLineupStat magiLineupSeason">今季</span><span class="magiLineupStat">${fmt(st.games)}試合</span><span class="magiLineupStat">${fmt(st.AB)}打数</span><span class="magiLineupStat">${fmt(st.H)}安打</span><span class="magiLineupStat">${fmt(st.RBI)}打点</span></div><div class="magiLineupStatsLine magiLineupRate"><span class="magiLineupStat">打率 <b>${fmt(st.AVG)}</b></span><span class="magiLineupStat">出塁率 <b>${fmt(st.OBP)}</b></span><span class="magiLineupStat">OPS <b>${fmt(st.OPS)}</b></span></div>`;
}
