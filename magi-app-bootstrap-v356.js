(()=>{
'use strict';
if(window.MAGI_APP_BOOTSTRAP_V356)return;
window.MAGI_APP_BOOTSTRAP_V356=true;
...
function quarantineButton(){
  const b=findButton();
  if(!b)return;
  if(b.id!=='magiRunButton')b.id='magiRunButton';
  if(b.type!=='button')b.type='button';
  if(!ready){
    if(!b.disabled)b.disabled=true;
    if(!failed&&b.textContent!=='MAGI準備中…')b.textContent='MAGI準備中…';
  }
}
