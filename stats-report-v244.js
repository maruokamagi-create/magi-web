(()=>{
'use strict';
if(window.MAGI_STATS_REPORT_V244)return;
const VERSION='v244';
...
function metricScore(r,y,m){
 const f=String(r?.fileName||''),s=String(r?.sheetName||''),all=`${f} ${s}`,sheet=n(s);
 let score=completeness(r);
 const rispSheet=/^得点圏打率一覧$/i.test(s.trim())||/得点圏打率一覧/i.test(s);
 if(m.key==='rispAvg')score+=rispSheet?600:-900;
 else if(rispSheet)score-=900;
 if(/通算成績一覧/i.test(f))score+=120;
 if(/丸岡中.*軟式野球部/i.test(f))score+=20;
 if(/通算|個人成績|打撃成績|打者成績|選手別成績|総合成績/i.test(s))score+=90;
 if(m.aliases.some(a=>{const x=n(a);return x.length>=2&&sheet.includes(x)}))score+=55;
 if(/得点圏|打順別|守備位置別|イニング別|対戦相手別|相手別|月別|球場別|左右別|カウント別|試合別/i.test(s))score-=160;
 if(/打撃詳細|投手詳細/i.test(all))score-=220;
...
function compositeRecord(candidates,y){
 ...
 for(const m of METRICS){
  const ranked=candidates.filter(r=>raw(r,m.aliases)!=='').map(r=>({r,score:metricScore(r,y,m)})).sort((a,b)=>b.score-a.score);
  const picked=ranked[0]?.r;
  values[m.key]=picked?raw(picked,m.aliases):'';
  if(picked)sources[m.key]={file:picked.fileName||'',sheet:picked.sheetName||''};
 }
 ...
 return{values,sources,sourceFiles,sourceSheets};
}
function collect(q){
 ...
 const records=ys.map(y=>{
  ...
  return{season:y,...merged,row:candidates[0]||null};
 }).filter(x=>x.row);
...
function render(result){
 ...
 panel.innerHTML=reportMarkup(result)+`<div class="statsPdfActions"><button id="statsPreviewButton" class="statsPdfButton" type="button">A4プレビュー</button><button id="statsSaveButton" class="secondary" type="button">PDFを作成</button><a id="statsPdfLink" class="statsPdfButton hidden" href="#">PDFを開く・共有</a><button id="statsPrintButton" class="secondary" type="button">印刷</button><div id="statsPdfState" class="statsPdfState">まずプレビューで内容を確認できます。PDFは必要な時だけ作成します。</div></div>`;
 ...
 document.getElementById('statsSaveButton')?.addEventListener('click',()=>createPdf(result));
 document.getElementById('statsPrintButton')?.addEventListener('click',printReport);
 const status=document.getElementById('status');if(status)status.textContent=`${result.target}の通算打撃成績を表示しました。「A4プレビュー」で配布資料を確認できます。`;
 panel.scrollIntoView({behavior:'smooth',block:'start'});
}
function installStyles(){
 if(document.getElementById('magi-stats-report-v244-style'))return;
...
window.MAGI_STATS_REPORT_V244=true;
