(()=>{
'use strict';
if(window.MAGI_BEST_ORDER_EVIDENCE_V346)return;
window.MAGI_BEST_ORDER_EVIDENCE_V346=true;
...
'大久保 陽翔は現チームのキャプテンであり精神的な柱。成績と同時にこの役割を必ず加味する。ただしキャプテンだから自動的に特定打順へ固定しない。主軸として評価したうえで、極端に調子を落としている場合は下位へ落とすだけでなく、近藤先生の起用思想として1番に置いて打席数を増やし復調を促す案も戦術的に比較する。',
...
'【9番・左翼】武田 晴琉翔はフライアウト処理と走力を近藤先生が評価しているため、現時点では9番・左翼の有力候補。ただし固定ではない。打撃の極端な不調、守備状態、怪我、他候補の上昇があれば変更する。9番起用は打撃だけでなく守備と走力、1番大野へのつながりまで含めて判断する。',
...
'チーム方針：中嶋玲月は中軸候補として評価する。大久保夢翔は能力を否定せず、役割を絞り、ミス・失点後の切り替えを観察しながら段階起用する。性格を固定評価しない。'
];
...
return{...(base||{}),count:PLAYERS.length,evidenceCounts:{base:Number(base?.count)||0,historical:old.length,current:cur.length,recentSix:recent.lines.length,fielding:pos.length},files:[...new Set([...(base?.files||[]),...all.map(r=>r.fileName).filter(Boolean)])],evidenceLayers:[...(base?.evidenceLayers||[]),'PAST PERFORMANCE / REPEATABILITY','CURRENT SEASON','RECENT SIX FORM','TEAM ROLE / CAPTAINCY','RATIONALE DEPTH','STANDARD TOP FIVE','FLEXIBLE 6-9','PRACTICE GAME FLEXIBILITY','RIGHT-HANDED DEFAULT','CURRENT FIELDING POLICY','ALL 14 PLAYERS','DEFENSIVE CONSTRAINTS'],summary:`過去実績${old.length}名・今季通算${cur.length}名・直近6集計${recent.lines.length}名・守備${pos.length}名に、標準上位5人の基準線、6〜9番の変動運用、右投手基本対応、練習試合の柔軟運用を重ねて1〜9番を審議する。`,text};}
function install(){if(typeof window.searchDataEvidence!=='function')return false;if(window.searchDataEvidence.__bestOrderV346)return true;const prev=window.searchDataEvidence;window.searchDataEvidence=function(q){const base=prev(q);return isBestOrder(q)?build(q,base):base};window.searchDataEvidence.__bestOrderV346=true;window.MAGI_BEST_ORDER_EVIDENCE_V268=true;return true}
