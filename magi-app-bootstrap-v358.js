(()=>{
'use strict';
if(window.MAGI_APP_BOOTSTRAP_V358)return;
window.MAGI_APP_BOOTSTRAP_V358=true;
...
function setError(message){failed=true;ready=false;quarantineButton();const b=findButton();if(b){b.disabled=true;b.textContent='MAGI準備エラー'}const st=document.getElementById('status');if(st)st.textContent=`正式MAGIを起動できませんでした：${message}`;console.error('[MAGI bootstrap v358]',message);}
...
function formalRunnerReady(){return typeof window.MAGI_FORMAL_UI_RUNNER_V3==='function'&&window.MAGI_FORMAL_UI_RUNNER_V3?.meta?.engineBoundaryEnforced===true&&window.MAGI_FORMAL_UI_RUNNER_V3?.meta?.numericFailClosed===true;}
...
async function boot(){
  ...
  await load(`/magi-formal-runner-v358.js?v=${REV}`);await waitCapability(formalRunnerReady);
  // main-live-answer-v329 still calls the stable V2 symbol. Point it at the v358
  // engine-boundary runner so there is exactly one semantic execution path.
  window.MAGI_FORMAL_UI_RUNNER_V2=window.MAGI_FORMAL_UI_RUNNER_V3;
  await load(`/main-live-answer-v329.js?v=${REV}`);await waitCapability(semanticReady);
  ready=true;activateButton();
  window.MAGI_APP_RUNTIME=Object.freeze({version:'canonical-app-v358',ready:true,entry:'GEMINI_SEMANTIC_FIRST',formalRunner:'V3_ENGINE_BOUNDARY_EVIDENCE',progress:'EVENT_DRIVEN_V358',numericFailClosed:true,localFallback:false,engineVersion:String(window.MAGI_ENGINE_V1?.version||'')});
